...
void calculate_new_centroids()
{
    ...
    for(long int i=0;i<N;i++)
    {
        long int cluster_current_point=clusters[i];
        centroids[index(cluster_current_point,0,2)]+=points[index(i,0,2)];
        centroids[index(cluster_current_point,1,2)]+=points[index(i,1,2)];
    }
